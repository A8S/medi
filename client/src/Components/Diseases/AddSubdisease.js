...
class AddSubdisease extends React.Component {
	...
	handleSubmit = async (e) => {
		e.preventDefault();
		let obj = { ...this.state };
		obj.books = JSON.stringify(this.state.books);
		obj.references = JSON.stringify(this.state.references);
		console.log(obj);
		const res = await createSubdisease(JSON.stringify(this.state), this.props.match.params.dId);
		console.log(res);
		// if (res.status == 200) {
				...
				<form onSubmit={this.handleSubmit}>
					<div className="form-group">
						...
						<div>
							...
							<button className="btn btn-primary btn-sm" onClick={this.onAddReference}>
								Add Reference
							</button>
